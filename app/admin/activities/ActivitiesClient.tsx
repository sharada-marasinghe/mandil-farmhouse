...
export default function ActivitiesClient({ initialActivities }: Props) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("action", editingId ? "UPDATE_ACTIVITY" : "CREATE_ACTIVITY");
      if (editingId) {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || `Failed to ${editingId ? "save" : "create"} activity.`);
      }
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. Boat Safari, Jet Ski Ride…"
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="Describe this activity, rules, maximum capacity..."
                rows={3}
                    ...
                    {existingImages.map((url, idx) => (
                      <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-slate-250 bg-slate-50">
                        ...
                        <button
                          ...
                          className="absolute top-1 right-1 w-5 h-5 rounded-full bg-red-50 text-red-650 flex items-center justify-center shadow-sm cursor-pointer border border-red-100"
                        >
                          <FiTrash2 size={9} />
                        </button>
                      </div>
            ...
            {error && <p className="text-xs text-red-655 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</p>}
            ...
            <div className="flex gap-3">
              {editingId && (
                <button
                  type="button"
                  onClick={handleCancelEdit}
                  className="flex-1 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-750 font-bold text-xs transition-colors cursor-pointer text-center"
                >
                  Cancel
                </button>
              )}
